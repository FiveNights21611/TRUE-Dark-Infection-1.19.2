				...
				if (DarkInfectionModVariables.WorldVariables.get(world).VoidKingSpawned == false) {
					if (!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 20, 20, 20), e -> true).isEmpty()) {
						...
						if (!world.isClientSide()) {
							...
							if (_mcserv != null)
								_mcserv.getPlayerList().broadcastSystemMessage(Component.literal("The Void King Has Spawned"), false);
						}
					...
					if (!world.isClientSide()) {
						MinecraftServer _mcserv = ServerLifecycleHooks.getCurrentServer();
						if (_mcserv != null)
							_mcserv.getPlayerList().broadcastSystemMessage(Component.literal("The Void King has already been found"), false);
					}
				}
			}
			world = _worldorig;
